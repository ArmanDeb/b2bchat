-- Fix conversations table constraints for group conversations
...
-- Update the constraint to allow NULL values for group conversations
DROP CONSTRAINT IF EXISTS check_participant_order ON public.conversations;
DROP CONSTRAINT IF EXISTS unique_conversation ON public.conversations;
...
-- Create unique constraint for one-on-one conversations only
ALTER TABLE public.conversations 
